...
// // min max
function product_min(){
  global $conn;
$sql = "SELECT * FROM sanpham WHERE DonGia  ORDER BY DonGia ASC";
  $resulf = mysqli_query($conn, $sql);
  $count=mysqli_num_rows($resulf);    
  if($count==0){
      return false;
...
// $sql = "SELECT MaSP, TenSP, DonGia FROM sanpham WHERE DonGia > 0 ORDER BY DonGia ASC";

// // Thực hiện truy vấn SQL
// $resulf=mysqli_query($conn,$sql);
// $count=mysqli_num_rows($resulf); 
// }
// $sqlHH3="SELECT hanghoa.MSHH, TenHH, Gia, TenHinh
// FROM hanghoa,hinhhh WHERE(hanghoa.MSHH=hinhhh.MSHH) ORDER BY Gia ASC Limit 10";
// $resultHH3 = mysqli_query($conn, $sqlHH3);
// lấy 1 product 
function product($id){
  global $conn;
