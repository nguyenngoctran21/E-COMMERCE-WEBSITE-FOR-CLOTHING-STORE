...
if(isset($_POST['order'])){
    ...
    if($order){
        ...
        $mail->Username   = 'nienluanart@gmail.com';        // tài khoản đăng nhập SMTP
        $mail->Password   = 'wvylahrbypqqwdhh';                         // mật khẩu đăng nhập SMTP
        $mail->SMTPSecure = 'tls';                                  // giao thức bảo mật TLS
        $mail->Port       = 587;
        $mail->setFrom('nienluanart@gmail.com', 'ART STORE');          // địa chỉ email và tên người gửi
        $mail->addAddress($kh['Email'], $kh['TenKH']); // địa chỉ email và tên người nhận
        $mail->Subject = ' ART STORE - DON HANG CUA BAN';                               // tiêu đề email
        $mail->Body    = $message;     
